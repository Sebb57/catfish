...
torch::Tensor Eval::fen_to_tensor(const std::string& fen)
{
...
float Eval::evaluate_board(torch::jit::script::Module& model, const std::string& fen)
{
    ...
    while (ss >> s) 
        tokens.push_back(s);
    if (tokens.size() < 6)
        throw EvalException("Invalid FEN string provided.");
